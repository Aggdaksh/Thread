...
- **Repo root** — this directory; contains `backend/`, `frontend/`, and `docs/`.
- **backend/** — Node.js HTTP + WebSocket server, MongoDB client, auth, observability.
- **frontend/** — React + Vite frontend; proxies `/api` and `/ws` to backend.
...
Build-time (Vite). Prefix: `VITE_*`. Set in `frontend/.env` or `.env.local` or when running `npm run dev`.
