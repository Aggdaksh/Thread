...
> Current deploy instructions live in [VERCEL_FRONTEND_RENDER_BACKEND.md](VERCEL_FRONTEND_RENDER_BACKEND.md) and [RENDER.md](RENDER.md). This audit is historical context; for the actual current build use `VITE_BACKEND_HTTP_URL` and `VITE_BACKEND_WS_URL`.
...
| Frontend build | Missing | No job to build `frontend` (e.g. `npm run build`). |
...
- Add a CI job to build the frontend (e.g. `cd frontend && npm ci && npm run build`) so breakages are caught.
...
| COOKIE_DOMAIN | Optional | Leave blank for default Vercel + Render URLs; use only for custom shared domains. |
...
| ROOT_ADMIN_EMAIL | Yes | Root admin email ensured at startup. |
| ROOT_ADMIN_PASSWORD | Yes | Root admin bootstrap password; secret. |
| DISABLE_REDIS | Optional | Set `true` for one-instance deploy without Redis. |
| REDIS_URL | Optional | Render Key Value internal URL when using Redis. |
...
- Document in deployment docs: “Frontend must set `VITE_BACKEND_WS_URL` to `wss://<backend-host>/ws`.”
...
| Item | Status | Notes |
|------|--------|-------|
| Static site | Not configured | If the frontend is a Vite SPA, it can be deployed as a Render “Static Site”: root directory `frontend`, build command `npm ci && npm run build`, publish directory `dist`. |
| Env at build time | Important | `VITE_BACKEND_HTTP_URL` and `VITE_BACKEND_WS_URL` must be set at build time so the built JS points to the backend URL (e.g. `https://<backend>.onrender.com` and `wss://<backend>.onrender.com/ws`). |
...
- Add a `render.yaml` section or doc for a “Static Site” service: build command, publish directory, and required env vars (`VITE_BACKEND_HTTP_URL`, `VITE_BACKEND_WS_URL`).
