...
/** Browser hides failed WS upgrade status (401/403) as close 1006. Validate HTTP session before reconnecting. */
let wsAuthCheckInFlight = false;
...
function getApiUrl(path) {
  const rawBase = import.meta.env?.VITE_BACKEND_HTTP_URL ?? "";
  const base = typeof rawBase === "string" ? rawBase.replace(/\/+$/, "") : "";
  return base ? `${base}${path}` : path;
}

async function getSessionStatusAfterWsFailure() {
  ...
  if (me.status !== 401 && me.status !== 403) return me.status;
...
function validateSessionBeforeReconnect(wsPath, closeCode) {
  if (wsAuthCheckInFlight) return;
  wsAuthCheckInFlight = true;
  getSessionStatusAfterWsFailure()
    .then((status) => {
      if (_shutdown || reconnectDisabled || ws != null) return;
      ...
      scheduleReconnect(wsPath, closeCode);
    })
    .catch(() => {
      if (!_shutdown && !reconnectDisabled && ws == null) {
        scheduleReconnect(wsPath, closeCode);
      }
    })
    .finally(() => {
      wsAuthCheckInFlight = false;
    });
}
...
function connect(wsPath = DEFAULT_WS_PATH) {
  ...
  ws.onclose = (ev) => {
    ...
    if (!wasReady && code === 1006) {
      setStatus("disconnected");
      validateSessionBeforeReconnect(wsPath, code);
      return;
    }
    setStatus("disconnected");
