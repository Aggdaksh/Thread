...
function getImageMessageSrc(content) {
  if (!content || typeof content !== "string") return null;
  const trimmed = content.trim();
  if (!trimmed) return null;
  const path = trimmed.split("?")[0].toLowerCase();
  if (!/\.(png|jpe?g|gif|webp)$/.test(path)) return null;
...
export function ChatWindow() {
  ...
  const sendChatContent = async (rawContent, { clearInput = false } = {}) => {
    ...
    if (!text || !user || connectionStatus === "connecting") return false;
  ...
  const handleSend = async () => {
    ...
    await sendChatContent(inputValue, { clearInput: true });
  };

  const handleAttachmentClick = () => {
    if (!conversationIdNormalized) {
      toast({ title: "Select a chat", description: "Choose a conversation before uploading an image.", variant: "destructive" });
      return;
    }
  ...
  const handleAttachmentChange = async (e) => {
    ...
    if (!ALLOWED_IMAGE_TYPES.has(file.type)) {
      toast({ title: "Unsupported file", description: "Upload JPG, PNG, GIF, or WebP images only.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      const sent = await sendChatContent(url);
      if (sent) {
        toast({ title: "Image sent", description: "Your image was uploaded successfully." });
      }
    } catch (err) {
              ...
              >
                ...
                <div className={cn("flex w-full max-w-3xl", isMe ? "ml-auto justify-end" : "justify-start")}>
                  ...
                  >
                    ...
                    ) : getImageMessageSrc(msg.content) ? (
                      <a
                        href={getImageMessageSrc(msg.content)}
                        target="_blank"
                        rel="noreferrer"
                        className="block p-1"
                      >
                        <img
                          src={getImageMessageSrc(msg.content)}
        ...
        <div className="max-w-4xl mx-auto flex items-end gap-2">
          <div className="relative flex-1 flex items-end gap-1">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="flex-shrink-0 self-center rounded-full h-11 w-11 text-muted-foreground flex items-center justify-center hover:bg-muted/70"
              onClick={handleAttachmentClick}
              disabled={!conversationIdNormalized || isUploadingAttachment || connectionStatus === "connecting"}
              data-testid="button-message-attachment"
              title="Attach image"
            >
              {isUploadingAttachment ? <Loader2 className="w-5 h-5 animate-spin" /> : <Paperclip className="w-6 h-6" />}
            </Button>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="inline-flex self-center items-center justify-center h-11 w-11 flex-shrink-0">
                <Button size="icon" className="rounded-full h-11 w-11 flex-shrink-0 flex items-center justify-center" onClick={handleSend} disabled={isSendDisabled} data-testid="button-send">
