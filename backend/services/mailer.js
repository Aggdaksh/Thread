...
async function sendMail(message) {
  const to = message && typeof message.to === 'string' ? message.to.trim() : '';
  ...
  const html = message && typeof message.html === 'string' ? message.html : text;

  if (!to || !subject || !text) {
    const err = new Error('sendMail requires to, subject, and text');
    err.code = 'MAIL_INVALID_MESSAGE';
    throw err;
  }
  ...
  if (!resend && !smtp) {
    const err = new Error('Mail service is not configured');
...
async function sendTestEmail(to) {
  const subject = 'Thread SMTP test';
  const text = 'Thread SMTP is configured correctly. If you received this email, password reset OTP emails can be delivered.';
  const html = '<p>Thread SMTP is configured correctly.</p><p>If you received this email, password reset OTP emails can be delivered.</p>';
  await sendMail({ to, subject, text, html });
}
