...
async function ensureRootAdmin() {
  ...
  if (user) {
    ...
    // Keep the root admin usable from the configured env password. This is
    // especially helpful in local/dev when the root account already exists.
    const passwordHash = await bcrypt.hash(password, SALT_ROUNDS);
    const passwordUpdated = await userStore.updatePasswordHash(user.id, passwordHash);
    if (!passwordUpdated && isProduction) {
      throw new Error('ensureRootAdmin: failed to update root password');
    }
