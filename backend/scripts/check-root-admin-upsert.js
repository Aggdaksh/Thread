...
async function run() {
  ...
  const username = (config.ROOT_ADMIN_USERNAME || 'daksh_root').trim();
  ...
  let root = await userStore.findByEmail(email);
  if (!root && username) {
    root = await userStore.findByUsername(username);
  }
  if (!root) {
    console.error('FAIL: root admin user not found after ensure by email or username');
    process.exit(1);
  }
